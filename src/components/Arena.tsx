...
export default function Arena({
    ...
    onBattleSave?: (record: import('../lib/battleHistory').BattleRecord) => void;
}) {
    const [battleStarted, setBattleStarted] = useState(false);

    const startBattle = () => {
        if (player1Graph && player2Graph) setBattleStarted(true);
    };

    const resetBattle = () => setBattleStarted(false);
    ...
    return (
        <div className="w-full flex justify-between gap-6">
            ...
            <div className="flex-1 bg-slate-900 border border-slate-700 rounded-3xl p-6 shadow-2xl relative overflow-hidden">
                ...
                <h2 className="text-2xl font-bold mb-4 text-blue-400">⚔ Player 1 Workspace</h2>
                <GraphPicker selectedGraph={player1Graph} onChange={onPlayer1Change} color="#60a5fa" />
            </div>

            {/* Center */}
            <div className="flex flex-col items-center justify-center gap-6 px-4">
                <div className="text-5xl font-black italic text-slate-700">VS</div>
                <button
                    onClick={startBattle}
                    disabled={!player1Graph || !player2Graph}
                    className="px-8 py-4 bg-gradient-to-r from-purple-600 to-pink-600 rounded-full font-bold text-lg uppercase tracking-widest shadow-[0_0_30px_rgba(219,39,119,0.5)] hover:scale-105 transition-all disabled:opacity-50 disabled:grayscale disabled:hover:scale-100"
                >
                    Initialize Battle
                </button>
                {(!player1Graph || !player2Graph) && (
                    <p className="text-xs text-slate-500 text-center max-w-[120px]">
                        Both players need a graph
                    </p>
                )}
            </div>
            ...
            <div className="flex-1 bg-slate-900 border border-slate-700 rounded-3xl p-6 shadow-2xl relative overflow-hidden">
                ...
                <h2 className="text-2xl font-bold mb-4 text-red-400">⚔ Player 2 Workspace</h2>
